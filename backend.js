...
const scenarios = {
  shortener: {
    ...
    estimateAnswer: "<p>Example: 100M new links/month ≈ 40 writes/s average. At 100 reads per write, redirects average ≈ 4K/s. For this exercise, assume a 5–10× peak; replace that placeholder with an observed peak-window distribution before capacity planning. At roughly 500 bytes per record, one year is about 600 GB of raw records before indexes, replication, allocator overhead, and backups. The read/write ratio alone does not justify caching: cache only if repeated or skewed lookups produce a useful hit rate and redirect semantics tolerate the chosen staleness.</p>",
    ...
    flowAnswer: "<p>Create: client → API → validation/abuse checks → ID generator → database → response. Redirect: edge/load balancer → redirect service → use a cache only after measured hot-key reuse and staleness policy justify it; on a miss—or when no cache is deployed—query the authoritative database, then redirect. Publish click events asynchronously for analytics.</p>",
    deepQuestion: "How are unique short codes generated without collisions or a central bottleneck?",
    deepAnswer: "<p>Options: generate a random Base62 candidate, attempt a conditional insert protected by the authoritative store’s unique constraint, and retry on conflict—never use a check-then-insert race. Durably allocated, non-overlapping numeric ranges encoded as Base62 avoid collisions only while the allocator prevents overlap and reuse. Snowflake-style IDs distribute generation but produce longer codes. Discuss predictability, collision probability, retry limits, allocator recovery, coordination, and code length.</p>",
  ...
  chat: {
    ...
    modelAnswer: "<p>WebSocket for live events plus HTTP for history/media. The durable server record includes <code>message_id, conversation_id, sender_id, server_sequence, client_idempotency_key, accepted_at, payload_ref</code>. The client supplies the idempotency key; the chat service assigns <code>server_sequence</code>. Order within a conversation, not globally.</p>",
    ...
    deepAnswer: "<p>Assign a monotonically increasing <code>server_sequence</code> atomically with the durable append for each conversation; clients must not choose the authoritative sequence. Consistently routing a conversation to one ordered log partition can serialize appends, but the service still owns the sequence and gap policy. Use client idempotency keys, acknowledge only after durability, allow duplicate delivery, and deduplicate on clients/servers.</p>",
